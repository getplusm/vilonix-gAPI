...
public class PrimoGUI extends Gui {

    public PrimoGUI(Player p, GuiDefaultContainer container) {
        super(p, container, "Возможности §7§lPRIMO");
    }
    ...
    protected void setItems() {
        ...
        dInventory.setItem(4, 3,
                new DItem(ItemUtil.getBuilder(Head.HOUSE).setName(ConfigUtil.getMessage("GUI_DONATE_GUI_ITEM_NAME"))
                        .setLore(ConfigUtil.getList("GUI_DONATE_PRIMO_ITEM_LORE")).build()));
        dInventory.setItem(6, 3, new DItem(ItemUtil.getBuilder(Head.FIRE).setName("§bGRIEF")
                .setLore(ConfigUtil.getList("GUI_DONATE_PRIMO_GRIEF_LORE")).build()));
    }
}
